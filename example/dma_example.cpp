...
int main(void) {
  ...
  for (volatile size_t j = 1; j < 5; j++) {
    // Allocate and configure DMA control block
    volatile auto* control_block = dma_memory.AllocateObject<volatile rpl::DmaControlBlock>();
    if (control_block == nullptr) {
      std::cerr << "Failed to allocate control block" << std::endl;
      // dma_memory.Free(const_cast< uint32_t*>(src_buffer));
      // dma_memory.Free(const_cast< uint32_t*>(dst_buffer));
      return 1;
    }
    ...
    volatile uint32_t* src_buffer = static_cast<volatile uint32_t*>(
        dma_memory.Allocate(kBufferSize * sizeof(uint32_t)));
    volatile uint32_t* dst_buffer = static_cast<volatile uint32_t*>(
        dma_memory.Allocate(kBufferSize * sizeof(uint32_t)));
    ...
    std::cout << "Source virtual: " << std::hex << src_buffer << ", physical: 0x"
              << src_physical << std::endl;
    std::cout << "Dest virtual: " << std::hex << dst_buffer << ", physical: 0x"
              << dst_physical << std::endl;


    std::cout << "Starting transfer " << j << "..." << std::endl;
    volatile uint32_t test_data[kBufferSize];
    for (size_t i = 1; i <= kBufferSize; i++) {
      test_data[i-1] = i * i;  // Sample test pattern
    }

    // !! Do not use memset due to alignment requirements !!
    for (volatile size_t i = 0; i < kBufferSize; i++) {
      // Initialize source buffer with test pattern
      src_buffer[i] = test_data[i] * j;
      // src_buffer[i] = (i+1) * (i+1) * j;
      // Clear destination buffer
      dst_buffer[i] = test_data[i] * j * 2;
    }
    ...
    for (size_t i = 0; i < kBufferSize; i++) {
      ...
      std::cout << src_buffer[i] << "\t" << dst_buffer[i]
                << std::endl;
