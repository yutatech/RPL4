...
namespace rpl
{
  ...
  struct SpiRegisterMap
  {
    ...
    struct CS
    {
      ...
      /**
       * @brief FIFO Clear
       * @note If CLEAR and TA are both set in the same operation, the FIFOs are cleared before the new frame is started. Read back as 0.
       */
      ...
      {
        kInactive = 0b0, // Transfer not active. /CS lines are all high (assuming CSPOL = 0). RXR and DONE are 0. Writes to SPI_FIFO write data into bits 15:0 of SPIDLEN and bits 7:0 of SPICS allowing DMA data blocks to set mode before sending data.
        kActive = 0b1,   // Transfer active. /CS lines are set according to CS bits and CSPOL. Writes to SPI_FIFO write data to TX FIFO. TA is cleared by a dma_frame_end pulse from the DMA controller.
      };

      /**
       * @brief DMA Enable
       * @note Peripheral generates data requests. These will be taken in four-byte words until the SPIDLEN has been reached.
       */
      ...
      {
        kDisable = 0b0, // Don’t automatically de-assert chip select at the end of a DMA transfer; chip select is manually controlled by software.
        kEnable = 0b1,  // Automatically de-assert chip select at the end of a DMA transfer (as determined by SPIDLEN)
      };

      /**
       * @brief Read Enable
       * @note Read enable if you are using bidirectional mode. If this bit is set, the SPI peripheral will be able to send data to this device.
       */
      ...
      {
        kInProgress = 0b0, // Transfer is in progress (or not active TA = 0).
        kCompleted = 0b1,  // Transfer is complete. Cleared by writing more data to the TX FIFO or setting TA to 0.
      };
      ...
      {
        kStillAvailable = 0b0, // RX FIFO is less than 3⁄4 full (or not active TA = 0).
        kNearlyFull = 0b1,     // RX FIFO is 3⁄4 or more full. Cleared by reading sufficient data from the RX FIFO or setting TA to 0.
      };
      ...
      {
        kNotFull = 0b0, // RX FIFO is not full.
        kFull = 0b1,    // RX FIFO is full. No further serial data will be sent / received until data is read from FIFO.
      };
      ...
      volatile CS_ cs : 2;            // Chip Select
      volatile CPHA cpha : 1;         // Clock Phase
      volatile CPOL cpol : 1;         // Clock Polarity
      volatile CLEAR clear : 2;       // FIFO Clear
      volatile CSPOL cspol : 1;       // Chip Select Polarity
      volatile TA ta : 1;             // Transfer Active
      volatile DMAEN dmaen : 1;       // DMA Enable
      volatile INTD intd : 1;         // Interrupt on Done
      volatile INTR intr : 1;         // Interrupt on RXR
      volatile ADCS adcs : 1;         // Automatically De-assert Chip Select
      volatile REN ren : 1;           // Read Enable
      volatile LEN len : 1;           // LoSSI enable
      volatile uint32_t lmono : 1;    // unused
      volatile uint32_t te_en : 1;    // unused
      volatile DONE done : 1;         // Transfer Done
      volatile RXD rxd : 1;           // RX FIFO contains Data
      volatile TXD txd : 1;           // TX FIFO can accept Data
      volatile RXR rxr : 1;           // RX FIFO needs Reading (3⁄4 full)
      volatile RXF rxf : 1;           // RX FIFO Full
      volatile CSPOL cspol0 : 1;      // Chip Select 0 Polarity
      volatile CSPOL cspol1 : 1;      // Chip Select 1 Polarity
      volatile CSPOL cspol2 : 1;      // Chip Select 2 Polarity
      volatile uint32_t dma_len : 1;  // Enable DMA mode in LoSSI mode
      volatile uint32_t len_long : 1; // Enable Long data word in LoSSI mode if DMA_LEN is set
    };

    /**
     * @brief SPI Master TX and RX FIFOs
     * @note This register allows TX data to be written to the TX FIFO and RX data to be read from the RX FIFO.
     */
    struct FIFO
    {
      /**
       * @brief DMA Mode (DMAEN set)
       * @note If TA is clear, the first 32-bit write to this register will control SPIDLEN and SPICS. Subsequent reads and writes will be taken as four-byte data words to be read/written to the FIFOs\n
       *       Poll/Interrupt Mode (DMAEN clear, TA set)\n
       *       Writes to the register write bytes to TX FIFO. Reads from register read bytes from the RX FIFO
       */
    ...
    struct DLEN
    {
       ...
       *       This field is only valid for DMA mode (DMAEN set) and controls how many bytes to transmit (and therefore receive).
       */
    ...
    {
      /**
       * @brief This sets the Output Hold delay in APB clocks. A value of 0 causes a 1 clock delay.
       */
     ...
     * @note This register controls the generation of the DREQ and Panic signals to an external DMA engine. The DREQ signals are generated when the FIFOs reach their defined levels and need servicing. The Panic signals instruct the external DMA engine to raise the priority of its AXI requests.
     */
    struct DC
    {
      /**
       * @brief DMA Read Panic Threshold.
       * @note Generate the Panic signal to the RX DMA engine whenever the RX FIFO level is greater than this amount.
       */
      volatile uint32_t tdreq : 8;

      /**
       * @brief DMA Read Request Threshold.
       * @note Generate a DREQ to the RX DMA engine whenever the RX FIFO level is greater than this amount (RX DREQ is also generated if the transfer has finished but the RX FIFO isn’t empty).
       */
      volatile uint32_t tpanic : 8;

      /**
       * @brief DMA Write Panic Threshold.
       * @note Generate the Panic signal to the TX DMA engine whenever the TX FIFO level is less than or equal to this amount.
       */
      volatile uint32_t rdreq : 8;

      /**
       * @brief DMA Write Request Threshold.
       * @note Generate a DREQ signal to the TX DMA engine whenever the TX FIFO level is less than or equal to this amount.
       */
